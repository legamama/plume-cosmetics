...
/**
 * Dynamic social icon component based on platform key
 */
export function SocialIcon({ type, size = 20, className }: SocialIconProps & { type: string }) {
    const IconComponent = socialIconMap[type.toLowerCase()];
    ...
    if (!IconComponent) {
        ...
        return (
            <span
                className={cn(
                    "inline-flex items-center justify-center font-medium text-xs uppercase",
                    className
                )}
